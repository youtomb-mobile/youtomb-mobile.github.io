...
function renderDataTrending(homeShelfTrendingType, shelfTitle) {
    ...
    if (window.location.hash.split("/").join(',').split("?").join(',').split(',').slice(1, 2)[0] !== "trending") {
    getHomeData2.open('GET', "https://corsproxy.org/?https://cvmapi.elijahr.dev/" + 'api/v2/chat?stuff=' + homeShelfTrendingType, true);
    } else if (window.location.hash.split("/").join(',').split("?").join(',').split(',').slice(1, 2)[0] == "trending") {
    getHomeData2.open('GET', "https://corsproxy.org/?https://cvmapi.elijahr.dev/" + 'api/v2/chat?thing=' + trendType, true);
    }
